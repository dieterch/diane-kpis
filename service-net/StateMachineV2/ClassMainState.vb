...
Public Class ClassMainState
    Public Sub New()
        AMM_List = New List(Of MSG)
        PrimaryMSGList = New List(Of PrimaryMSG)
        BGWCalcState = New BackgroundWorker With {.WorkerReportsProgress = True}
        Silence = True
        Bussy = False
        GapMaxLength = 5400 'in Sekunden, 5400 = 1,5h
        _TaretloadreachTime = 300
        _Alarm_Delay_Time = 1000
    End Sub
...
#Region "Private Variablen"
    ...
    Private E_Action As New EngineAction
    Private PrimaryMSG As MSG = Nothing
    Private WithEvents BGWCalcState As BackgroundWorker
    Private AMM_List As List(Of MSG)
...
#Region "Properties"
    ...
    Public WriteOnly Property EngineMSG_List As List(Of MSG)
        Set(value As List(Of MSG))
            Try
                _Bussy = True
                'AMM_List.Clear()
                AMM_List = value
                _Status_List = New List(Of ActionDB)
                If AMM_List Is Nothing OrElse AMM_List.Count = 0 Then
                    RaiseEvent ErrorInClass("No Messages in EngineMSG_List")
                    _Bussy = False
                    RaiseEvent Calc_Finisched(_Status_List)
                ElseIf _PrimaryMSGList Is Nothing OrElse _PrimaryMSGList.Count = 0 Then
                    RaiseEvent ErrorInClass("PrimaryMSGList is empty")
                    _Bussy = False
                    RaiseEvent Calc_Finisched(_Status_List)
                Else
                    E_Action = New EngineAction
                    BGWCalcState.RunWorkerAsync()
                End If
            Catch ex As Exception
                RaiseEvent ErrorInClass("Error 33 " & ex.Message)
                BGWCalcState.ReportProgress(-1, "Error 33 " & ex.Message)
            End Try
        End Set
    End Property
    <CategoryAttribute("Input"), Description("Primary Trip List")>
    Public Property PrimaryMSGList As List(Of PrimaryMSG)
    <CategoryAttribute("Input"), Description("waiting time in ms if an additional Alarm is happen. Default 1500ms")>
    Public Property Alarm_Delay_Time As Integer
    <CategoryAttribute("Input"), Description("If Silence is TRUE (DEFAULT), no message is generated if primary Message is not found")>
    Public Property Silence As Boolean
    <CategoryAttribute("Input"), Description("Max time difference in seconds between two MSG. Default is 5400 sec. (1,5h)")>
    Public Property GapMaxLength As Integer
    ...
    Public Property TaretloadreachTime As Integer
